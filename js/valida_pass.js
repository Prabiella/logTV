  ...
  var progressM = $('#passwordProgressM');
  ...
  function validatePwd() {
    ...
    progressM.css('width', (checks.length - errors.length) * (100 / checks.length) + '%' );
    updateProgressColorM((checks.length - errors.length) * (100 / checks.length));
  ...
  function validatePwdM() {
    var errors = [];
    var pM = pwdM.val();
    
    validation.html("");
    for(var i = 0; i < checks.length; i++){
      if(!pM.match(checks[i].regex)){
        errors.push(checks[i].msg);
        validation.append('<li class="validation-rule">'+checks[i].msg+'</div>');
      }
    }
  ...
  function updateProgressColorM(width) {
    var color;
    if (width >= 100) {
      color = '#336600';
      statusM.innerHTML = "Segura";
      statusM.style.color ='#336600';
    } else if (width >= 60) {
      color = '#FF8029';
      statusM.innerHTML = "";
      statusM.style.color ='#FF8029';
    } else {
      statusM.innerHTML = "";
      statusM.style.color ='#BC0000';
      color = '#BC0000';
    }
    progressM.css('background-color', color);
  }

  pwdM.on('keyup change', validatePwdM);
...
function viewPasswordM() {
  var passwordInput = document.getElementById('passwordM');
  var passStatus = document.getElementById('pass-statusM');

  if (passwordInput.type == 'passwordM'){
    passwordInput.type='text';
    passStatus.className='fa fa-eye-slash'; 
  } else {
    passwordInput.type='passwordM';
